...
TRANSLATIONS = {
    'uz': {
        ...
        'thermal_decay_label': "Termal Degradatsiya (β):",  # added for translation
...
# Tensile ratio slider removed – not used in calculations; tensile strength taken from HB or manual
...
if PYVISTA_AVAILABLE:
    ...
    if not PT_AVAILABLE:
        ...
        **Mohr-Coulomb failure criterion** asosida **Deep Bayesian Inference**.  
        ...
        with col_b1:
            t_in = st.slider("Harorat (°C)", 20, 1100, 500, key="pinn_t")
            s_v = st.slider("Vertikal kuchlanish (MPa)", 5, 60, 30, key="pinn_sv")
            c_in = st.slider("Jins cohesion (MPa)", 5, 25, 12, key="pinn_c")
        with col_b2:
            phi_in = st.slider("Ishqalanish burchagi (°)", 15, 50, 32, key="pinn_phi")
            p_in = st.slider("G‘ovak bosimi (MPa)", 0, 30, 10, key="pinn_pp")
        ...
        if run_pinn:
            ...
            c3.metric("Maʼlumot shovqini (Aleatoric)", f"{a:.4f}")

            st.info(f"""
            **Tahlil natijasi**:
            - **Xavf**: {m*100:.1f}% – umumiy buzilish ehtimoli.
            - **Model bilim darajasi**: ±{e*100:.2f}% – ushbu nuqtada model qanchalik ishonchli?
            - **Sensor aniqligi**: {a:.4f} – kuzatilgan maʼlumotlarning ichki noaniqligi.
            """)
...
with st.expander("🌍 3D Litologik Kesim"):
    ...
    active_wells_3d = states_132[stage]  # stage defined above
...
with st.expander("🎲 Monte Carlo Noaniqlik Tahlili"):
    ...
    with mc_col2:
        fos_mc, pf = monte_carlo_fos(layers_data[-1]['ucs'], ucs_std_val,
                                     ...
                                     layers_data[-1]['mi'],  # pass actual mi
