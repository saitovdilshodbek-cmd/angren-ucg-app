...
# =========================== MATEMATIK METODOLOGIYA (KESH BILAN EMAS, CHUNKI O‘ZGARMAS) ===========================
...
# =========================== QATLAM MA'LUMOTLARI (VERTIKAL JOYLASHTIRISH) ===========================
...
# =========================== KESHLANGAN HARORAT MAYDONI FUNKSIYASI ===========================
    ...
    for val in sources.values():
        if time_h > val['start']:
            ...
            Q_heat += (curr_T / 10.0) * np.exp(
                -((grid_x - val['x'])**2 + (grid_z - source_z)**2) / (2 * 30**2))
...
# =========================== GRID VA MANBA ===========================
# Grid o‘lchamlarini aniqlaymiz (keshlash uchun kerak)
z_axis_len = 80
x_axis_len = 100
grid_shape = (z_axis_len, x_axis_len)
...
# Maksimal harorat maydonini sessiyada saqlash
...
with c2:
    ...
    # Selek joylarini chizish
    sources_pos = {'1': -total_depth/3, '2': 0, '3': total_depth/3}
    for px in sources_pos.values():
...
with tab_live:
    ...
    if run_live:
        ...
        for t_step in range(TIME_STEPS):
            ...
            # Tarixni cheklash (MAX_HISTORY=1000)
            MAX_HISTORY = 1000
