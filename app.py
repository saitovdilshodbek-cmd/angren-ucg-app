"""
UCG Geomechanical Monitor — To'liq to'g'irlangan versiya
Barcha xatolar tuzatildi:
  1. layers_data bo'sh bo'lsa crash — himoya qo'shildi
  2. FDM iteratsiya time_h ga bog'liq qilindi
  3. Gaz oqimi vizualizatsiyasi — quiver (scatter arrows) bilan almashtrildi
  4. session_state reset logikasi — elif bilan to'g'irlandi
  5. st.table → pd.DataFrame bilan
  6. _objective: risk w ga bog'liq qilindi (void fraction w bo'yicha interpolyatsiya)
  7. RandomForestRegressor → RandomForestClassifier
"""
...
if formula_option != "Yopish":
    with st.expander(f"📚 Ilmiy asos: {formula_option}", expanded=True):
        if formula_option == "1. Hoek-Brown Failure (2018)":
            st.latex(
                r"\sigma_1 = \sigma_3 + \sigma_{ci} \left( m_b \frac{\sigma_3}{\sigma_{ci}} + s \right)^a"
            )
            st.latex(
                r"m_b = m_i \exp\left(\frac{GSI-100}{28-14D}\right);\quad"
                r"s = \exp\left(\frac{GSI-100}{9-3D}\right)"
            )
            st.latex(
                r"a = \frac{1}{2} + \frac{1}{6}\left(e^{-GSI/15} - e^{-20/3}\right)"
            )
            st.info(
                "**Hoek-Brown:** GSI va Disturbance (D) faktorlari asosida massiv mustahkamligini hisoblash."
            )
            ...
            st.info(
                "**Termal degradatsiya:** Harorat ta'sirida jins strukturasining emirilishi va o'tkazuvchanlik ortishi."
            )
        elif formula_option == "3. Thermal Stress & Tension":
            st.latex(
                r"\sigma_{th} = \frac{E \cdot \alpha \cdot \Delta T}{1 - \nu} + \xi \cdot \nabla T"
            )
            st.latex(
                r"\sigma_{t(T)} = \sigma_{t0} \cdot \exp\left(-\beta_{th} (T - 20)\right)"
            )
            st.latex(r"FOS = \frac{\sigma_{limit}}{\sigma_{actual}}")
            st.info(
                "**Termo-mexanika:** Termal kengayish kuchlanishi va cho'zilish mustahkamligining kamayishi."
            )
        elif formula_option == "4. Pillar & Subsidence":
            st.latex(
                r"\sigma_{p} = (UCS \cdot \eta) \cdot \left( \frac{w}{H} \right)^{0.5}"
            )
            st.latex(
                r"y = \frac{H}{2} \left( \sqrt{\frac{\sigma_v}{\sigma_p}} - 1 \right)"
            )
            st.latex(
                r"S(x) = S_{max} \cdot \exp\left( -\frac{x^2}{2i^2} \right);\quad"
                r"\epsilon = 1.52 \frac{S(x)}{R}"
            )
            st.info(
                "**Geomexanika:** Selek barqarorligi, plastik zona va yer yuzasining gorizontal deformatsiyasi."
            )
...
num_layers   = st.sidebar.number_input(
    "Qatlamlar soni:", min_value=1, max_value=5, value=3
)
tensile_mode = st.sidebar.selectbox(
    "Tensile modeli:", ["Empirical (UCS)", "HB-based (auto)", "Manual"]
)
...
beta_thermal  = st.sidebar.number_input(
    "Thermal Decay (β):", value=0.0035, format="%.4f"
)
...
for i in range(int(num_layers)):
    ...
    with st.sidebar.expander(
        f"{i+1}-qatlam parametrlari", expanded=(i == int(num_layers) - 1)
    ):
        ...
        color = st.color_picker(
            f"Rangi:", strata_colors[i % len(strata_colors)], key=f"color_{i}"
        )
        g     = st.slider(f"GSI:", 10, 100, _gsi_def, key=f"g_{i}")
        m     = st.number_input(f"mi:", value=_mi_def, key=f"m_{i}")
        s_t0_val = (
            st.number_input(f"σt0 (MPa):", value=3.0, key=f"st_{i}")
...
# ==============================================================================
# FIX #1 — layers_data bo'sh bo'lsa dasturni to'xtatish
# ==============================================================================
if not layers_data:
...
(
    ...
    grid_s_hb,
    grid_a_hb,
    grid_sigma_t0_manual,
) = [np.zeros_like(grid_z) for _ in range(6)]

# ==============================================================================
# FIX #4 — session_state reset: elif bilan to'g'irlandi
# ==============================================================================
if (
    "max_temp_map" not in st.session_state
...
for i, layer in enumerate(layers_data):
    ...
    overburden = (
        sum(l["rho"] * 9.81 * l["t"] for l in layers_data[:i]) / 1e6
    )
    grid_sigma_v[mask] = (
        overburden + (layer["rho"] * 9.81 * (grid_z[mask] - layer["z_start"])) / 1e6
    )
    grid_ucs[mask]  = layer["ucs"]
    grid_mb[mask]   = layer["mi"] * np.exp(
        (layer["gsi"] - 100) / (28 - 14 * D_factor)
    )
    grid_s_hb[mask] = np.exp((layer["gsi"] - 100) / (9 - 3 * D_factor))
    grid_a_hb[mask] = 0.5 + (1 / 6) * (
        np.exp(-layer["gsi"] / 15) - np.exp(-20 / 3)
    )
    grid_sigma_t0_manual[mask] = layer["sigma_t0_manual"]

# ==============================================================================
# --- 🔥 GAUSSAN HARORAT TARQALISHI ---
# ==============================================================================
alpha_rock = 1.0e-6
sources = {
...
# ==============================================================================
# FIX #2 — FDM Heat Solver: iteratsiya soni time_h ga bog'liq
# ==============================================================================
...
# Stability: alpha=1e-6, dx=1.0 → dt_max = 0.25/alpha = 250_000 s
# Har bir soat uchun dt=3600 s qadamlari
DX        = 5.0           # m (grid spacing)
DT        = 100.0         # s (vaqt qadami, stability: r = 1e-6*100/25 = 4e-6 ✅)
n_steps   = max(1, int(time_h * 3600 / DT))
# Hisoblash vaqtini cheklash: maksimum 200 qadam (interaktiv uchun)
...
st.session_state.max_temp_map = np.maximum(
    st.session_state.max_temp_map, temp_2d
)
...
# Failure zonalari
tensile_failure = (
    (sigma3_act <= -sigma_t_field_eff)
    & (delta_T > 50)
    & (sigma1_act > sigma3_act)
)
sigma3_safe  = np.maximum(sigma3_act, 0.01)
sigma1_limit = sigma3_safe + sigma_ci * (
    grid_mb * sigma3_safe / (sigma_ci + 1e-6) + grid_s_hb
) ** grid_a_hb
...
void_volume = np.sum(void_mask_permanent) * (
    (x_axis[1] - x_axis[0]) * (z_axis[1] - z_axis[0])
)

# ==============================================================================
# FIX #3 — Gaz oqimi: Darcy + Quiver (scatter arrows) vizualizatsiyasi
# ==============================================================================
def gas_flow(k: np.ndarray, pressure: np.ndarray):
...
pressure_field = temp_2d * 10.0
vx_gas, vz_gas = gas_flow(perm, pressure_field)
gas_velocity   = np.sqrt(vx_gas ** 2 + vz_gas ** 2)

# Quiver uchun subsampling (har 10-chi nuqta)
step = 10
qx   = grid_x[::step, ::step].flatten()
qz   = grid_z[::step, ::step].flatten()
qu   = vx_gas[::step, ::step].flatten()
qw   = vz_gas[::step, ::step].flatten()
qmag = gas_velocity[::step, ::step].flatten()

# Normalize arrows
arrow_scale = 8.0
qmag_max    = qmag.max() + 1e-30
qu_n        = qu / qmag_max * arrow_scale
qw_n        = qw / qmag_max * arrow_scale

# Void sohasida kuchlanish nolga
...
# ==============================================================================
# FIX #7 — AI MODEL: RandomForestClassifier (binary classifier)
# ==============================================================================
try:
...
for _ in range(15):
    ...
    y_zone_calc = (H_seam / 2) * (
        np.sqrt(sv_seam / (p_strength + 1e-6)) - 1
    )
...
# ==============================================================================
# FIX #6 — AI optimizatsiya: risk ham w ga bog'liq
# ==============================================================================
# void_fraction_base — hozirgi holat uchun baza
void_fraction_base = float(np.mean(void_mask_permanent))


def _objective(w_arr: np.ndarray) -> float:
    """
    Minimizatsiya funksiyasi.
    Selek kengligi w ga bog'liq:
      - strength: kengroq selek → kuchliroq
      - risk:     kengroq selek → kamroq void (pillar hajmi ortadi)
    """
    w = w_arr[0]
    strength = (ucs_seam * strength_red) * (w / H_seam) ** 0.5
    # Kengroq selek → void_fraction pasayadi (hisoblash modeli)
    risk = void_fraction_base * np.exp(-0.01 * (w - rec_width))
...
m5.metric(
    "AI Tavsiya (Selek)",
    f"{optimal_width_ai:.1f} m",
    delta=f"Klassik: {rec_width} m",
    delta_color="off",
)
...
uplift = (
    (total_depth * 1e-4)
    * np.exp(-(x_axis ** 2) / (total_depth * 10))
    * (time_h / 150)
    * 100
)
...
with col_g3:
    ...
    s_s  = grid_s_hb.max()
    a_s  = grid_a_hb.max()
    s1_20      = sigma3_ax + ucs_seam * (
        mb_s * sigma3_ax / (ucs_seam + 1e-6) + s_s
    ) ** a_s
    ucs_burn   = ucs_seam * np.exp(-0.0025 * (T_source_max - 20))
    s1_burning = sigma3_ax + ucs_burn * (
        mb_s * sigma3_ax / (ucs_burn + 1e-6) + s_s
    ) ** a_s
    s1_sov     = sigma3_ax + (ucs_seam * strength_red) * (
        mb_s * sigma3_ax / (ucs_seam * strength_red + 1e-6) + s_s
    ) ** a_s
    fig_hb = go.Figure()
    fig_hb.add_trace(
        go.Scatter(
            x=sigma3_ax, y=s1_20, name="20°C", line=dict(color="red", width=2)
        )
    )
    fig_hb.add_trace(
        go.Scatter(
...
with c2:
    ...
    fig_tm = make_subplots(
        ...
        subplot_titles=(
            "Harorat Maydoni (°C) + Gaz Oqimi Yo'nalishi",
    ...
    # FIX #3 — Gaz oqimi: quiver (scatter arrows) bilan ko'rsatish
    # Har bir nuqtada kichik chiziq segment (annotationlardan tezroq)
    fig_tm.add_trace(
        go.Scatter(
            x=qx,
            y=qz,
            ...
            marker=dict(
                ...
                color=qmag,
                colorscale="Blues",
                angle=np.degrees(np.arctan2(qw_n, qu_n)),
                opacity=0.7,
                showscale=False,
            ),
            name="Gaz oqimi",
    ...
    # --- ROW 2: AI Collapse Prediction (Viridis) ---
    fig_tm.add_trace(
        go.Heatmap(
            z=collapse_pred,
            x=x_axis,
...
# ==============================================================================
# --- 🌀 UCG: 3D DINAMIK MODEL ---
# ==============================================================================
st.header("🌀 UCG: Termo-Mexanik Dinamik 3D Model")


def generate_hourly_3d_model(h: float, layers: list) -> go.Figure:
    grid_res  = 45
    ...
    for i, cx in enumerate(centers_x):
        ...
        if h <= start_h:
            radii.append(0)
            temp_states.append("Sovuq")
        elif start_h < h <= start_h + 40:
            radii.append(2 + (h - start_h) / 40 * 10)
            temp_states.append("Faol")
        else:
            radii.append(12)
            temp_states.append("Soviyotgan")

    total_subs = np.zeros_like(gx3)
    for i in range(3):
        if radii[i] > 0:
            amp = (radii[i] / 12) * 5.0 * min(h / 150, 1.0)
            total_subs += -amp * np.exp(
                -((gx3 - centers_x[i]) ** 2 + gy3 ** 2) / 600
            )
    ...
    coal_z_c = -(
        sum(l["t"] for l in layers[:-1]) + layers[-1]["t"] / 2
    )
    ...
    if "Faol" in temp_states:
        ...
        n_pts = int(5 + (h % 30))
        st_x  = rng.uniform(centers_x[act_i] - 20, centers_x[act_i] + 20, n_pts)
        st_y  = rng.uniform(-20, 20, n_pts)
        st_z  = rng.uniform(coal_z_c, coal_z_c + 15, n_pts)
        fig.add_trace(
            go.Scatter3d(
                x=st_x,
                y=st_y,
    ...
    fig.update_layout(
        scene=dict(
            ...
            zaxis=dict(
                title="Chuqurlik Z (m)", range=[-total_h_depth - 10, 20]
            ),
            aspectmode="manual",
...
st.info(
    f"ℹ️ Sidebar'dagi jarayon vaqti ({time_h}-soat) asosida "
...
def calculate_live_metrics(h: float, layers: list, T_max: float):
    target    = layers[-1]
    ucs_0_l   = target["ucs"]
    H_l       = target["t"]
    curr_T_l  = (
        25 + (T_max - 25) * (min(h, 40) / 40)
        if h <= 40
...
p_str, w_rec_live, t_now, s_max_3d = calculate_live_metrics(
...
def generate_integrated_3d(h: float, layers: list, s_max: float) -> go.Figure:
    grid_res = 35
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(
                title="X (m)", backgroundcolor="black", showbackground=True
            ),
            yaxis=dict(
                title="Y",
                range=[-60, 20],
...
with col_left:
    ...
    st.plotly_chart(
        generate_integrated_3d(time_h, layers_data, s_max_3d),
        use_container_width=True,
    )
...
with col_right:
    ...
    st_trend = [
        calculate_live_metrics(v, layers_data, T_source_max)[0] for v in h_axis
    ]
    fig_trend = go.Figure()
    fig_trend.add_trace(
        go.Scatter(
            x=h_axis, y=st_trend, name="Mustahkamlik",
...
with st.expander("📝 Avtomatik Ilmiy Interpretatsiya", expanded=True):
    risk_level = (
        "YUQORI" if p_str < 15 else "O'RTA" if p_str < 25 else "PAST"
    )
...
H_depth_tot  = (
    sum(l["t"] for l in layers_data[:-1]) + target_l["t"] / 2
)
...
t1, t2, t3 = st.tabs(
    ["🏗️ Massiv Parametrlari", "🔥 Termal Degradatsiya", "⚖️ Barqarorlik & Manbalar"]
)
...
with t2:
    ...
    # FIX #5 — st.table pd.DataFrame bilan
    params_df = pd.DataFrame(
        {
            "Parametr":         ["Elastiklik Moduli (E)", "Termal kengayish (α)", "Boshlang'ich T₀"],
            "Qiymat":           [f"{E_MODULUS} MPa", f"{ALPHA_T_COEFF} 1/°C", "20 °C"],
            "Tanlanish sababi": [
    ...
    st.latex(
        r"\sigma_{ci(T)} = \sigma_{ci(0)} \cdot e^{-\beta(T-T_0)} = "
        + f"{ucs_t_dyn:.2f}"
        + r" \text{ MPa}"
    )
    st.write(
        f"**Interpretatsiya:** {T_source_max}°C haroratda jins mustahkamligi "
        f"{((1 - ucs_t_dyn / ucs_0_r) * 100):.1f}% ga pasaydi."
    )
    st.markdown("**B) Termal kuchlanish ($\\sigma_{th}$):**")
    st.latex(
        r"\sigma_{th} \approx \frac{E \cdot \alpha \cdot \Delta T}{1 - \nu} = "
        + f"{sigma_thermal.max():.2f}"
        + r" \text{ MPa}"
    )
...
with t3:
    ...
    st.write(
        f"Selek o'lchami $w={rec_width}$ m bo'lganda, uning markaziy yadrosi "
    ...
    if fos_final < 1.3:
        st.error(
            f"🔴 **Ilmiy Xulosa:** FOS={fos_final:.2f}. Termal degradatsiya yuqori. "
            f"Selek enini oshirish yoki gazlashtirish tezligini nazorat qilish tavsiya etiladi."
        )
    else:
        st.success(
            f"🟢 **Ilmiy Xulosa:** FOS={fos_final:.2f}. "
