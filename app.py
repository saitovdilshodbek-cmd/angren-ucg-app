...
# Til tanlash menyusi (sidebarning eng yuqorisiga qo‘yamiz)
...
def t(key, **kwargs):
    ...
    translations = {
        'uz': {
            ...
            'layer_params': "{i+1}-qatlam parametrlari",
            'layer_name': "Nomi:",
            ...
            'hb_mb': "m_b = {:.3f}",
            'hb_s': "s = {:.4f}",
            'hb_caption_mb': "Massiv ishqalanish burchagi funksiyasi ($m_i={}$)",
            'hb_caption_s': "Yoriqlilik darajasi (GSI={})",
            ...
            'ucs_decay_eq': r"\sigma_{{ci(T)}} = \sigma_{{ci(0)}} \cdot e^{{-\beta(T-T_0)}} = {:.2f} \text{{ MPa}}",
            ...
            'fos_eq': r"FOS = \frac{{\sigma_p}}{{\sigma_v}} = {:.2f}",
...
| **Integratsiya va testlash** | 2026-06-30 | Unit testlar, yakuniy vizualizatsiya, deploy |
        ...
        'en': {
            ...
            'layer_params': "Layer {i+1} parameters",
            'layer_name': "Name:",
            ...
            'hb_mb': "m_b = {:.3f}",
            'hb_s': "s = {:.4f}",
            'hb_caption_mb': "Mass friction angle function (m_i={})",
            'hb_caption_s': "Fracturing degree (GSI={})",
...
| **Integration & testing** | 2026-06-30 | Unit tests, final visualization, deploy |
        ...
        'ru': {
            ...
            'layer_params': "Параметры слоя {i+1}",
            'layer_name': "Название:",
            ...
            'hb_mb': "m_b = {:.3f}",
            'hb_s': "s = {:.4f}",
            'hb_caption_mb': "Функция угла трения массива (m_i={})",
            'hb_caption_s': "Степень трещиноватости (GSI={})",
            ...
            'ucs_decay_eq': r"\sigma_{{ci(T)}} = \sigma_{{ci(0)}} \cdot e^{{-\beta(T-T_0)}} = {:.2f} \text{{ МПа}}",
...
| **Интеграция и тестирование** | 2026-06-30 | Модульные тесты, финальная визуализация, деплой |
    ...
    if kwargs:
        # qo'llab-quvvatlash uchun: {placeholder} format
...
formula_option = st.sidebar.selectbox(
    t('tensile_model'),  # bu yerda aslida formula_option uchun emas, lekin originalda shunday edi. To'g'rilaymiz.
    ["Yopish", "1. Hoek-Brown Failure (2018)",
     "2. Thermal Damage & Permeability",
     "3. Thermal Stress & Tension",
     "4. Pillar & Subsidence"]
)
# (formula_option uchun tarjima kerak emas, chunki bu ilmiy nomlar)
...
for i in range(int(num_layers)):
    with st.sidebar.expander(t('layer_params', i+1=i+1), expanded=(i == int(num_layers) - 1)):
...
with t1:
    ...
    with c1r:
        st.latex(t('hb_mb', mb_dyn))
        st.caption(t('hb_caption_mb', mi_val))
        st.latex(t('hb_s', s_dyn))
        st.caption(t('hb_caption_s', gsi_val))
    with c2r:
