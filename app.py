# ============================================================
# app_final.py - Barcha tuzatishlar bilan birlashtirilgan ilova
# ============================================================
...
EPS = 1e-9  # ✅ Nolga bo'linishni oldini oladi, eng boshida aniqlansin
...
except ImportError:          # ✅ Faqat import xatosini tut
    PT_AVAILABLE = False
    device = "cpu"
...
def compute_temperature_field_moving(time_h, T_source_max, burn_duration, total_depth, source_z, grid_shape, n_steps=20):
    ...
    # FDM diffuziya (CFL tekshiruvli)
    dx = float(x_axis[1] - x_axis[0])
    dz = float(z_axis[1] - z_axis[0])
    dt = 3600.0
    cfl_x = alpha_rock * dt / dx**2
    cfl_z = alpha_rock * dt / dz**2
    cfl_max = max(cfl_x, cfl_z)
    if cfl_max > 0.25:
        scale = int(np.ceil(cfl_max / 0.25))
        n_steps = n_steps * scale
        dt_sub = dt / scale
        alpha_eff = alpha_rock * dt_sub / dx**2
    else:
        alpha_eff = alpha_rock * dt / dx**2
    T = temp_2d.copy()
...
# void_factor xavfsiz qo'llaniladi
void_factor = np.where(void_mask_permanent, 0.1, 1.0)
sigma1_void = sigma1_act * void_factor
sigma3_void = sigma3_act * void_factor
sigma_ci_void = sigma_ci * void_factor
...
def generate_ucg_dataset_fixed(n=10000):
    """Vektorlashtirilgan, np.maximum bilan"""
    rng = np.random.default_rng(42)
    T  = rng.uniform(20,   1000, n)
    s1 = rng.uniform(0,    50,   n)
    s3 = rng.uniform(0,    30,   n)
    d  = rng.uniform(0,    300,  n)
    damage   = 1 - np.exp(-0.002 * np.maximum(T - 100, 0))
    strength = 40 * (1 - damage)
    collapse = ((s1 > strength) | (T > 700)).astype(int)
    return np.column_stack([T, s1, s3, d, collapse])

@st.cache_resource
def get_nn_model_fixed():
    if not PT_AVAILABLE: return None
    model = CollapseNet().to(device)
    try:
        model.load_state_dict(torch.load("collapse_model.pth", map_location=device, weights_only=True))
        model.eval()
        return model
    except (FileNotFoundError, RuntimeError):
        data = generate_ucg_dataset_fixed()
...
if nn_model is not None:
    ...
    try:
        collapse_pred = predict_nn(nn_model, temp_2d, sigma1_void, sigma3_void, grid_z)
    except:
        collapse_pred = np.zeros_like(temp_2d)
else:
    X_ai = np.column_stack([temp_2d.flatten(), sigma1_void.flatten(), sigma3_void.flatten(), grid_z.flatten()])
...
# FOS xaritasi (void_factor hisobga olingan holda)
fos_2d = np.clip(sigma1_limit/(sigma1_void+EPS), 0, 3.0)
...
m5.metric(t('ai_recommendation'), f"{rec_width:.1f} m", delta=f"AI optimal: {rec_width:.1f} m")

# Cho'kish va HB grafiklar
st.markdown("---")
...
with c2:
    ...
    def compute_advanced_fos(grid_x, grid_z, active_wells, well_x, source_z, h_seam, cavity_width,
        ...
        if stage == 2:
            ...
            pillar_mask = (np.abs(grid_x - well_x[1]) < (selek_eni / 2)) & \
                          (np.abs(grid_z - source_z) < (h_seam / 2))
            fos[pillar_mask] = 2.5
...
with tab_test:
    ...
    def add(x, y):
        return x + y
    def test_add():
        assert add(1, 2) == 3,   f"1+2={add(1,2)} bo'lishi kerak 3"
        assert add(-1, -2) == -3, f"-1+(-2)={add(-1,-2)} bo'lishi kerak -3"
        assert add(0, 0) == 0,   f"0+0={add(0,0)} bo'lishi kerak 0"
...
with tab_errors:
    ...
    if lang_choice == "Python":
        st.code('''# Sintaksis xatosi (':') yo'q
# if x == 5 print(x)  # noto'g'ri

# Tuzatilgan:
if x == 5:
    print(x)''', language="python")
        st.code('''# Mantiqiy xato: butun bo'linish
def average(nums):
    total = sum(nums)
    return total // len(nums)  # noto'g'ri

# Tuzatilgan:
def average(nums):
    total = sum(nums)
    return total / len(nums)''', language="python")
        st.code('''# Resurs oqishi: fayl yopilmagan
# def read_file(path):
#     f = open(path, 'r')
#     data = f.read()
#     # f.close() chaqirilmagan

# Tuzatilgan:
def read_file(path):
    with open(path, 'r') as f:
        return f.read()''', language="python")
    # ... (boshqa tillar o'xshash)
    
with tab_tools:
    st.subheader("Tillar bo‘yicha linter / formatter va debugger vositalari")
    tools_data = [
        {"Til": "Python", "Linter / Formatter": "Pylint, Flake8, Black", "Buyruq": "pylint code.py\nflake8 code.py\nblack code.py", "Debugger": "PDB, ipdb"},
        {"Til": "JavaScript", "Linter / Formatter": "ESLint, Prettier", "Buyruq": "npx eslint file.js\nnpx prettier --write .", "Debugger": "node inspect, Chrome DevTools"},
        {"Til": "Java", "Linter / Formatter": "Checkstyle, SpotBugs", "Buyruq": "mvn checkstyle:check\nmvn spotbugs:spotbugs", "Debugger": "jdb, IDE"},
        {"Til": "C/C++", "Linter / Formatter": "clang-format, cppcheck", "Buyruq": "clang-format -i file.cpp\ncppcheck .", "Debugger": "gdb, Valgrind"},
        {"Til": "Go", "Linter / Formatter": "go fmt, go vet", "Buyruq": "go fmt ./...\ngo vet ./...", "Debugger": "Delve"},
        {"Til": "Rust", "Linter / Formatter": "rustfmt, Clippy", "Buyruq": "cargo fmt --check\ncargo clippy", "Debugger": "gdb, rust-lldb"},
        {"Til": "PHP", "Linter / Formatter": "phpcs", "Buyruq": "./vendor/bin/phpcs src/", "Debugger": "Xdebug"}
    ]
