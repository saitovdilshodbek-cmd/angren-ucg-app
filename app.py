...
with col_info1:
    st.metric("O'rtacha Harorat (Kamera zonasi)", f"{avg_temp_at_source:.1f} °C")
with col_info2:
    st.metric("Mustahkamlik Pasayishi", f"{thermal_reduction*100:.1f} %")
with col_info3:
    st.metric("Tavsiya etilgan Selek Eni", f"{recommended_width:.1f} m", help="Termo-mexanik kuchlanishni hisobga olgan xolda xavfsiz kenglik")

st.markdown("---")
col_g1, col_g2, col_g3 = st.columns([1.5, 1.5, 2])

# Subsidence va Termal
...
with col_g2:
    fig2 = go.Figure(go.Scatter(x=x_axis, y=uplift := (total_depth * 1e-4) * np.exp(-(x_axis**2) / (total_depth*10)) * (time_h/150) * 100, fill='tozeroy', line=dict(color='cyan', width=3)))
...
with col_g3:
    ...
    reduction_val = np.exp(-0.002 * (T_source_max - 20))
    s1_init = sigma3_axis + avg_ucs * (mb * sigma3_axis / avg_ucs + s_hb)**a_hb
    s1_hot = sigma3_axis + (avg_ucs * reduction_val) * (mb * sigma3_axis / (avg_ucs * reduction_val) + s_hb)**a_hb
    fig_hb = go.Figure()
    fig_hb.add_trace(go.Scatter(x=sigma3_axis, y=s1_init, name='Normal', line=dict(color='#FF4B4B')))
    fig_hb.add_trace(go.Scatter(x=sigma3_axis, y=s1_hot, name='Termal Effekt', line=dict(color='#FFA500', width=4)))
...
with c1:
    st.subheader("🧱 Selek Monitoringi")
...
with c2:
    ...
    # Tavsiya etilgan selek chegaralarini vizual ko'rsatish
    for px in [p_x1, p_x2]:
        fig_tm.add_shape(type="rect", x0=px-recommended_width/2, x1=px+recommended_width/2, y0=source_z-10, y1=source_z+10, 
