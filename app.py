...
# 2. GEOTEXNIK TAHLIL (Siz so'ragan formulalar)
...
with c2:
    # !!! BU YERDA O'ZGARTIRISH KIRITILDI (O'ZINGIZDAGI SHU QISMNI ALMASHTIRING) !!!
    ...
    # --- 1-grafik: Harorat ( go.Contour ishlatildi) ---
    fig_tm.add_trace(go.Contour(
        ...
        zmin=25, 
        zmax=T_source_max,
        contours=dict(coloring='heatmap', showlines=True), # Izochiziqlar qo'shildi
        colorbar=dict(title="°C", x=1.02, y=0.78, len=0.45)
    ), row=1, col=1)
    
    # --- 2-grafik: Failure Index ( go.Contour ishlatildi) ---
    fig_tm.add_trace(go.Contour(
        z=failure_index_2d, 
        x=x_axis, 
        y=z_axis[:,0], 
