...
Barcha 100 ta ekspert tuzatish qo'llangan + 16 ta qo'shimcha tuzatma.
...
# [XATO #1] Biot koeffitsienti funksiyasi (doimiy emas)
...
# [XATO #3] Sutherland konstantalari gaz turlari uchun
SUTHERLAND_PARAMS = {
    "CO": {"S": 118.0, "mu_ref": 1.74e-5, "T_ref": 273.15},
    "CO2": {"S": 240.0, "mu_ref": 1.39e-5, "T_ref": 273.15},
    "CH4": {"S": 140.0, "mu_ref": 1.11e-5, "T_ref": 273.15},
    "H2": {"S": 87.0, "mu_ref": 8.76e-6, "T_ref": 273.15},
}
...
# [XATO #2] Termal degradatsiya eksponensial modeli
def thermal_degradation_gsi(gsi_0: float, temp: float, beta: float = BETA_GSI_DEFAULT) -> float:
    """
    Termal degradatsiya qoya mexanikasi uchun (eksponensial)

    Modeli: GSI(T) = GSI₀ × exp(-β(T-T_ref)/T_ref)
    Manba: Shao et al. (2003) + ishlangan tajribalar
    """
    temp_diff = max(temp - T_REF_AMBIENT, 0.0)
    decay_factor = np.exp(-beta * temp_diff / T_REF_AMBIENT)
    return float(np.clip(gsi_0 * decay_factor, 10.0, 100.0))


# [FIX #5] GSI termal degradatsiyasi uchun eski funksiya yangisi bilan almashtiriladi
def gsi_thermal_degradation(gsi_0: float, T: float, T_ref: float = T_REF_AMBIENT, beta_gsi: float = BETA_GSI_DEFAULT) -> float:
    """Yangilangan termal degradatsiya funksiyasi"""
    return thermal_degradation_gsi(gsi_0, T, beta_gsi)
...
# [XATO #4] Hoek-Brown parametrlarini to'g'ri qaytaruvchi funksiya
...
FORMULA_OPTIONS = {
    "uz": ["Yopish", "1. Hoek-Brown Failure (2018)", "2. Thermal Damage & Permeability", "3. Thermal Stress & Tension", "4. Pillar & Subsidence"],
    "en": ["Close", "1. Hoek-Brown Failure (2018)", "2. Thermal Damage & Permeability", "3. Thermal Stress & Tension", "4. Pillar & Subsidence"],
    "ru": ["Закрыть", "1. Разрушение Хука-Брауна (2018)", "2. Термическое повреждение и проницаемость", "3. Термическое напряжение и растяжение", "4. Целик и оседание"],
}
...
# [XATO #3] yangi gaz viskoziteti funksiyasi
def gas_viscosity_temperature(T_kelvin: np.ndarray, gas_type: str = "CO") -> np.ndarray:
    mu = np.zeros_like(T_kelvin, dtype=float)
    for i, Tk in enumerate(np.nditer(T_kelvin)):
        mu.flat[i] = sutherland_viscosity(gas_type, float(Tk))
    ...
    with warnings.catch_warnings():
        ...
        for step_i in range(n_steps):
            ...
            T_new[1:-1, 1:-1] = T_old[1:-1, 1:-1] + dt * (
                alpha_field[1:-1, 1:-1] * (Txx + Tzz) + Q[1:-1, 1:-1] / (rho_field[1:-1, 1:-1] * cp_field[1:-1, 1:-1] + EPS_GENERAL)
            )
            T_new[:, 0] = T_new[:, 1]
            T_new[:, -1] = T_new[:, -2]
            T_new[-1, :] = T_new[-2, :]
...
def kirsch_stress_field(
    x: np.ndarray, z: np.ndarray, sigma_H: float, sigma_h: float, cavity_radius: float, pore_pressure: float = 0.0
...
def propagate_uncertainty_analytical(
    ...
    dfos_ducs = (
        _quick_fos(ucs_mean * (1 + eps_rel), gsi_mean, T_mean, H_seam, rec_width, d_factor, beta_th, depth, rho) - fos_base
    ) / (eps_rel * ucs_mean + EPS_GENERAL)

    dfos_dgsi = (
        _quick_fos(ucs_mean, gsi_mean * (1 + eps_rel), T_mean, H_seam, rec_width, d_factor, beta_th, depth, rho) - fos_base
    ) / (eps_rel * gsi_mean + EPS_GENERAL)

    dfos_dT = (
        _quick_fos(ucs_mean, gsi_mean, T_mean * (1 + eps_rel), H_seam, rec_width, d_factor, beta_th, depth, rho) - fos_base
...
def _train_models(X, y, sigma1, sigma_ci, temp, damage):
    ...
    if PT_AVAILABLE:
        model = train_hybrid_model(
            X_train_sc, y_train, sigma1[idx_train], sigma_ci[idx_train], temp[idx_train], damage[idx_train]
        )
...
def compute_advanced_fos(
    ...
    for px_idx in active_wells_tuple:
        ...
        for layer_idx, (top, bot, layer) in enumerate(layer_bounds_list):
            ...
            # [XATO #2] Yangi termal degradatsiya funksiyasi
            ...
            if np.any(local_thermal):
                grad_T_local = np.sqrt(
                    np.gradient(T, axis=1, edge_order=2)[mask] ** 2 + np.gradient(T, axis=0, edge_order=2)[mask] ** 2
                )
...
# [XATO #2] gsi_thermal_degradation allaqachon yangi eksponensial model bilan qayta aniqlangan
# (yuqoridagi funksiyaga qarang)
...
# [XATO #8] Yangilangan digital_twin_hash funksiyasi
def digital_twin_hash(params_dict: dict) -> str:
    ...
    for k, v in params_dict.items():
        if isinstance(v, float):
            rounded_params[k] = round(v, 6)
        elif isinstance(v, dict):
            rounded_params[k] = {kk: round(vv, 6) if isinstance(vv, float) else vv for kk, vv in v.items()}
        else:
            rounded_params[k] = v
    serialized = json.dumps(rounded_params, sort_keys=True, default=str)
...
for lyr in layers_data:
    if lyr["thickness"] <= 0 and t("error_thick_positive") not in seen_errors:
        errors.append(t("error_thick_positive"))
        seen_errors.add(t("error_thick_positive"))
    if lyr["ucs"] <= 0 and t("error_ucs_positive") not in seen_errors:
        errors.append(t("error_ucs_positive"))
        seen_errors.add(t("error_ucs_positive"))
    if lyr["rho"] <= 0 and t("error_density_positive") not in seen_errors:
        errors.append(t("error_density_positive"))
        seen_errors.add(t("error_density_positive"))
    if not (10 <= lyr["gsi"] <= 100) and t("error_gsi_range") not in seen_errors:
        errors.append(t("error_gsi_range"))
        seen_errors.add(t("error_gsi_range"))
    if lyr["mi"] <= 0 and t("error_mi_positive") not in seen_errors:
        errors.append(t("error_mi_positive"))
        seen_errors.add(t("error_mi_positive"))
...
fos_mean_unc, fos_std_unc = propagate_uncertainty_analytical(
    ucs_seam, 0.10, float(target_layer["gsi"]), 5.0, T_source_max, 50.0, H_seam, rec_width, D_factor, beta_thermal, depth_seam, avg_rho
)
...
hybrid_model, rf_model, scaler, X_test_ai, y_test_ai = get_ensemble_model_cached(
    fingerprint, X_ai, y_ai, sigma1_act.flatten(), sigma_ci_flat, temp_2d.flatten(), thermal_damage(temp_2d.flatten(), beta_thermal)
)
...
with c2:
    ...
    fig_tm.add_trace(
        go.Heatmap(z=temp_2d, x=x_axis, y=z_axis, colorscale="Hot", zmin=25, zmax=T_source_max, colorbar=dict(title="T (°C)", x=1.05, y=0.78, len=0.42, thickness=15), name=t("temp_subplot")),
        row=1,
        col=1,
    )
    ...
    if np.any(mask_q):
        ...
        fig_tm.add_trace(
            go.Scatter(
                ...
                y=qz[mask_q],
                mode="markers",
                marker=dict(symbol="arrow", size=10, color=qmag[mask_q], colorscale="ice", cmin=0, cmax=qmag_max, angle=angles, opacity=0.85, showscale=False, line=dict(width=0)),
                name=t("gas_flow"),
            ),
            row=1,
            col=1,
    ...
    for idx_w in active_wells:
        ...
        fig_tm.add_shape(
            ...
            x0=px - r_burn_vis,
            x1=px + r_burn_vis,
            y0=source_z - r_burn_vis,
            y1=source_z + r_burn_vis,
    ...
    for px in well_x_pos:
        fig_tm.add_shape(
            ...
            x0=px - rec_width / 2,
            x1=px + rec_width / 2,
            y0=source_z - H_seam / 2,
            y1=source_z + H_seam / 2,
...
with st.expander("📊 Uncertainty Quantification (UQ) — FOS"):
    ...
    fos_samp = np.array(
        [
            float(apply_thermal_degradation(u, T, beta_thermal)) * (WILSON_C1 + WILSON_C2 * rec_width / (H_seam + EPS_STRESS)) / (vertical_stress(depth_seam, avg_rho) + EPS_STRESS)
...
if PYDOE_AVAILABLE:
    with st.expander("🎲 Latin Hypercube Sampling"):
        ...
        fos_lhs = np.clip(
            np.array([float(apply_thermal_degradation(u, T, beta_thermal)) for u, T in zip(UCS_lhs, T_lhs)]) / (np.vectorize(lambda d: vertical_stress(d, avg_rho))(Depth_lhs) + EPS_STRESS),
            0.0,
            10.0,
        )
...
with st.expander("📈 FOS Time Forecast (Trend Analysis)"):
    ...
    if len(time_points) < 2:
        ...
        for ct in time_points:
            T_at_ct = (
                T_REF_AMBIENT + (T_source_max - T_REF_AMBIENT) * min(ct, burn_duration) / max(burn_duration, 1)
                if burn_duration > 0
                else T_REF_AMBIENT
            )
...
with st.expander("🎲 Monte Carlo Uncertainty Analysis"):
    ...
    with mc_col2:
        fos_mc, pf_mc = monte_carlo_fos(
            layers_data[-1]["ucs"], ucs_std_val, layers_data[-1]["gsi"], gsi_std_val, layers_data[-1]["mi"], D_factor, avg_t_p, H_seam, depth_seam, avg_rho, rec_width, beta_thermal, n_sim=int(n_mc)
        )
        fig_mc = go.Figure()
    ...
    mc_stats = pd.DataFrame(
        {
            ...
            "Value": [
                f"{np.mean(fos_mc):.3f}",
                f"{np.median(fos_mc):.3f}",
                f"{np.std(fos_mc):.3f}",
                f"{np.percentile(fos_mc, 5):.3f}",
                f"{np.percentile(fos_mc, 95):.3f}",
                f"{pf_mc * 100:.2f}%",
            ],
        }
    )
...
with st.expander("🌪️ Sensitivity Analysis (Tornado Plot)"):
    df_sens, fos_base_sa = sensitivity_analysis(
        layers_data[-1]["ucs"], layers_data[-1]["gsi"], D_factor, nu_poisson, avg_t_p, H_seam, beta_thermal, depth_seam, avg_rho
    )
        ...
        with st.spinner("Generating ISRM/ISO report..."):
            try:
                ...
                docx_bytes = generate_full_iso_report(
                    ...
                    T_source_max=T_source_max,
                    burn_duration=float(burn_duration),
                    pillar_strength=pillar_strength_creep,
                    analytical_width=analytical_width,
                    fos_2d=fos_worst_case,
                    risk_map=risk_index_var,
                    void_volume=void_volume,
                    prepared_by=prepared_inp,
                    approved_by=approved_inp,
                    doc_number=doc_num_input,
                    revision=revision_inp,
...
with tab_live:
    ...
    if run_live:
        ...
        while not st.session_state.stop_live and steps_done < TIME_STEPS:
            ...
            with subs_ph.container():
                st.plotly_chart(
                    go.Figure(go.Heatmap(z=Z_subs * 100.0, x=X_live, y=Y_live, colorscale="Viridis")).update_layout(title="Surface Subsidence (cm)", height=300, template="plotly_dark"),
                    use_container_width=True,
                )
            with temp_ph.container():
                st.plotly_chart(
                    go.Figure(go.Heatmap(z=Z_temp, x=X_live, y=Y_live, colorscale="Hot")).update_layout(title="Temperature Field (°C)", height=300, template="plotly_dark"),
                    use_container_width=True,
                )
            pillar_ph.metric("Pillar Width (m)", f"{pillar_w_pred:.2f}", delta=f"FOS={FOS_live:.2f}")
            with trend_ph.container():
                st.plotly_chart(
                    go.Figure(go.Scatter(y=subs_hist, mode="lines+markers")).update_layout(title="Subsidence Trend", height=300, template="plotly_dark"),
                    use_container_width=True,
                )
            ...
            with surface_ph.container():
                ...
                if anom_pts[0].size > 0:
                    surf_fig.add_trace(
                        go.Scatter3d(
                            x=X_grid_live[anom_pts],
                            y=Y_grid_live[anom_pts],
                            z=Z_subs[anom_pts] * 100.0,
                            mode="markers",
            ...
            alerts_list = []
            if FOS_live < 1.2:
                alerts_list.append("⚠️ FOS Critical!")
            if mean_subs_v * 100.0 > 3.0:
                alerts_list.append("⚠️ High Subsidence!")
            if float(np.max(Z_temp)) > 1100.0:
                alerts_list.append("🔥 Overheating Alert!")
    ...
    with st.expander("🌐 3D Subsidence Surface (FIX #77)", expanded=False):
        ...
        Z_3d_subs = (
            -float(Smax) * float(1.0 - np.exp(-float(c_subs) * float(time_h))) * np.exp(-R_3d**2 / (2.0 * float(influence_radius) ** 2)) * 100.0
        )
        ...
        fig_3d.update_layout(
            ...
            scene=dict(xaxis_title="X (m)", yaxis_title="Y (m)", zaxis_title="Cho'kish (cm)", camera=dict(eye=dict(x=1.5, y=1.5, z=1.0))),
            ...
            for step_ai in range(int(ai_steps_1)):
                ...
                with ph1.container():
                    ...
                    fig_a = make_subplots(
                        rows=2,
                        cols=2,
                        subplot_titles=("Eff. Stress & Anomalies", "Temperature (°C)", "Gas Pressure (MPa)", "Stress (MPa)"),
            ...
            for i_tab2 in range(int(ai_steps_2)):
                ...
                with ph2.container():
                    ...
                    fig_fos_t2.add_trace(
                        go.Scatter(
                            x=T_sim[: i_tab2 + 1], y=sigma_v_sim[: i_tab2 + 1], mode="markers", marker=dict(color=list(range(i_tab2 + 1)), colorscale="Viridis", size=6)
                        ),
                        row=1,
                        col=2,
                    )
                    ...
                    try:
                        ...
                        if _fos_scaler is None:
                            ...
                            saved_dir = save_models_to_disk(
                                st.session_state.get("fos_nn_model"), st.session_state.get("fos_rf_tab2"), _fos_scaler, obj_name, _fos_meta
                            )
...
with tab_advanced:
    ...
    with t3_adv:
        ...
        # [XATO #1] Biot koeffitsienti funksiyasidan foydalanish
        biot_alpha = compute_biot_coefficient(saturation_ratio=0.8)  # taxminiy suv to'yinganlik
    ...
    with st.expander("📜 Patent Claims (UzPatent + PCT) — [FIX #86, #90, #92, #93]", expanded=False):
        ...
        st.info(
            ...
            "Versiya: 2.0.0-PhD | Fixes: 100 + 16 | Date: 2026-06-12"
        )
...
# [XATO #9] Pre-computed animation frames
@st.cache_data(show_spinner=False)
def get_animation_frames_cached(t_steps, fos_d, disp_d, h_disp, v_disp, x_ax, z_ax, surf_x):
    return create_animation_frames(t_steps, fos_d, disp_d, h_disp, v_disp, x_ax, z_ax, surf_x)
...
def draw_interactive_dashboard(x_ax, z_ax, fos_d, disp_d, surf_x, h_disp, v_disp, t_steps, fos_thr=1.0, cscale="Turbo"):
    fig_d = make_subplots(
        rows=2,
        cols=2,
        subplot_titles=("A) FOS & Yielded Zones", "B) Total Displacement (cm)", "C) H-Disp Surface History", "D) V-Disp Surface History"),
        horizontal_spacing=0.1,
        vertical_spacing=0.15,
    )
    fos_colorscale_seg = [
    ...
    fig_d.add_trace(
        go.Heatmap(
            ...
            zmin=0,
            zmax=3,
            colorbar=dict(title="FOS", x=0.45, y=0.78, thickness=12, len=0.42, tickvals=[0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0], ticktext=["0", "0.5", "1.0", "1.5", "2.0", "2.5", "3.0+"]),
            name="FOS",
        ),
        row=1,
        col=1,
    ...
    fig_d.add_trace(
        go.Heatmap(z=disp_d, x=x_ax, y=z_ax, colorscale=cscale, colorbar=dict(title="Disp (cm)", x=1.0, y=0.78, thickness=12, len=0.42)), row=1, col=2
    )
    fig_d.add_trace(
        go.Heatmap(
            z=h_disp[0:1, :],
            ...
            y=[t_steps[0]],
            colorscale="Turbo",
            zmin=float(np.min(h_disp)),
            zmax=float(np.max(h_disp)),
            showscale=False,
        ),
        row=2,
        col=1,
    ...
    fig_d.add_trace(
        go.Heatmap(
            ...
            y=[t_steps[0]],
            colorscale="Viridis",
            zmin=float(np.min(v_disp)),
            zmax=float(np.max(v_disp)),
            showscale=False,
        ),
        row=2,
        col=2,
    ...
    frames = get_animation_frames_cached(tuple(t_steps), fos_d, disp_d, h_disp, v_disp, x_ax, z_ax, surf_x)
    fig_d.frames = frames
    fig_d.update_layout(
        title=dict(text="Interactive UCG Monitoring Dashboard", x=0.5, font=dict(size=20, color="white")),
        template="plotly_dark",
        height=900,
        showlegend=False,
        ...
        updatemenus=[
            dict(
                ...
                yanchor="top",
                buttons=[
                    dict(label="▶ Play", method="animate", args=[None, {"frame": {"duration": 500, "redraw": True}, "fromcurrent": True, "transition": {"duration": 0}}]),
        ...
        sliders=[
            dict(
                steps=[
                    dict(
                        method="animate",
                        args=[[f"f{k}"], {"mode": "immediate", "frame": {"duration": 300, "redraw": True}, "transition": {"duration": 0}}],
                        label=f"{v:.0f}h",
                    )
                    for k, v in enumerate(t_steps)
                    if k % 3 == 0
                ],
                active=0,
                y=0.0,
...
# [XATO #15] Versiya ma'lumotlari
version_info = get_version_info()
st.sidebar.write(f"Version: {version_info['version']} (PhD-grade) | Fixes: 100 + 16")
...
# [XATO #16] Kengaytirilgan litsenziya matni
st.sidebar.warning(LICENSE_TEXT)

# ── Asosiy footer ──────────────────────────────────────────────────────────
st.markdown("---")
st.caption(
    f"**UCG SCI-Grade Platform v{version_info['version']}** | 100/100 Expert Fixes + 16 Applied | "
    f"Build: {version_info['build']} | Commit: {version_info['commit']} | "
    f"© 2026 Saitov Dilshodbek, Tashkent Technical University | "
    f"Patent Pending (UzPatent + WIPO PCT) | "
    f"⚠️ Scientific use only — Commercial use strictly prohibited until patent grant."
)

# ══════════════════════════════════════════════════════════════════════════════
# UCG SCI-GRADE PLATFORM v3.0.0 — TUZATISHLAR JADVALI (FIX C seriyasi + 16 qo'shimcha)
# ══════════════════════════════════════════════════════════════════════════════
# C-01: sigma_v_coal /=1e6 (Pa→MPa) — tuzatilgan
...
# C-06: digital_twin_hash — MD5→SHA-256
...
#
# QO'SHIMCHA 16 TA TUZATMA:
# 1. Biot koeffitsienti funksiyasi (doimiy emas)
# 2. Termal degradatsiya eksponensial modeli
# 3. Sutherland parametrlari gaz turlari uchun
# 4. Hoek-Brown parametrlarini to'g'ri qaytaruvchi funksiya
# 5. SQL input validatsiyasi
# 6. Caching yaxshilash (session state)
# 7. Exception handling yaxshilash (PyTorch seed)
# 8. Digital twin hash rounding va SHA-256
# 9. Pre-computed animation frames caching
# 10. NumPy array copying o'rniga view ishlatish
# 11. ML modelni diskka saqlash (mavjud)
# 12. ReproducibilityManager klassi
# 13. Docstring formatini yaxshilash
# 14. Patent hujjatlari (fayllar yaratish eslatmasi)
# 15. Versiya tasdiqlash
# 16. Kengaytirilgan litsenziya matni
# ══════════════════════════════════════════════════════════════════════════════
