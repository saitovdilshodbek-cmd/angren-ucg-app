...
# --- Session state (elif bilan to'g'ri reset) ---
...
# ==============================================================================
pressure     = temp_2d * 10.0          # soddalashtirilgan P-T bog'lanish
...
# ==============================================================================
# --- 🧠 AI MODEL — RandomForestClassifier (FIX: Regressor→Classifier) ---
# ==============================================================================
try:
...
# ==============================================================================
# --- ⚙️ SELEK OPTIMIZATSIYASI (Klassik + AI SLSQP) ---
# ==============================================================================
...
# --- AI SLSQP optimizatsiya (FIX: risk w ga bog'liq) ---
void_frac_base = float(np.mean(void_mask_permanent))

def objective(w_arr: np.ndarray) -> float:
    """
    Kengaytirilgan maqsad funksiya:
      - strength: kengroq selek → kuchliroq pillar
      - risk:     kengroq selek → void kamayadi (eksponensial)
    """
    w        = w_arr[0]
...
with c2:
    ...
    # --- ROW 2: AI Collapse Prediction (Viridis, opacity=0.4) ---
    fig_tm.add_trace(go.Heatmap(
...
def calculate_live_metrics(h, layers, T_max):
    ...
    ucs_0     = target['ucs']
    H_l       = target['t']
    curr_T    = (25 + (T_max - 25) * (min(h, 40) / 40)
...
def generate_integrated_3d(h, layers, s_max):
    ...
    for i, layer in enumerate(layers):
        ...
        fig.add_trace(go.Surface(x=gx, y=gy, z=z_top, colorscale=cs,
                                 opacity=0.9, showscale=False, name=layer['name'], hoverinfo='skip'))
        fig.add_trace(go.Surface(x=gx, y=gy, z=z_bottom, colorscale=cs,
    ...
    if r_c > 1:
        ...
        fig.add_trace(go.Surface(
            ...
            y=(r_c * 0.7) * np.sin(u_c) * np.sin(v_c),
            z=(r_c * 0.5) * np.cos(v_c) + coal_z,
    ...
    fig.update_layout(
        scene=dict(
            ...
            aspectmode='manual', aspectratio=dict(x=1, y=0.6, z=0.5)
        ),
