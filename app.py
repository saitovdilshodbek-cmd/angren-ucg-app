...
def generate_hourly_3d_model(h, layers):
    ...
    for i in range(3):
        if radii[i] > 0:
            ...
            total_subs += -amplitude * np.exp(-((grid_x_3d - centers_x[i])**2 + grid_y_3d**2) / 600)
    ...
    for i, layer in enumerate(layers):
        ...
        fig.add_trace(go.Surface(
            x=grid_x_3d, y=grid_y_3d, z=-z_top + layer_deform,
        ...
        current_depth += layer['t']

    coal_z_center = -(sum(l['t'] for l in layers[:-1]) + layers[-1]['t']/2)
    u_ang, v_ang = np.mgrid[0:2*np.pi:18j, 0:np.pi:18j]
    for i, cx in enumerate(centers_x):
        if radii[i] > 0:
            r = radii[i]
            c_map = 'YlOrRd' if temp_states[i] == "Faol" else 'Greys'
            opac  = 0.9     if temp_states[i] == "Faol" else 0.6
            dx_c = r * np.cos(u_ang) * np.sin(v_ang)
            dy_c = (r * 0.8) * np.sin(u_ang) * np.sin(v_ang)
            dz_c = (r * 0.5) * np.cos(v_ang) + coal_z_center
            fig.add_trace(go.Surface(x=dx_c + cx, y=dy_c, z=dz_c, colorscale=c_map, opacity=opac, showscale=False, name=f"Kamera {i+1}"))
    ...
    if "Faol" in temp_states:
        ...
        fig.add_trace(go.Scatter3d(x=st_x, y=st_y, z=st_z, mode='markers', marker=dict(size=4, color='cyan', symbol='diamond'), name="Termal Kuchlanish"))
...
def calculate_live_metrics(h, layers, T_max):
    ...
    ucs_0_l = target['ucs']
    H_seam_l = target['t']
    curr_T_l = 25 + (T_max - 25) * (min(h, 40) / 40) if h <= 40 else T_max * np.exp(-0.001 * (h-40))
    strength_red_l = np.exp(-0.0025 * (curr_T_l - 20))
    w_rec_l = 15.0 + (h / 150) * 10
    p_strength_l = (ucs_0_l * strength_red_l) * (w_rec_l / H_seam_l)**0.5
    max_sub_val_l = (H_seam_l * 0.05) * (min(h, 120) / 120)
    return p_strength_l, w_rec_l, curr_T_l, max_sub_val_l


p_str, w_rec, t_now, s_max_3d = calculate_live_metrics(time_h, layers_data, T_source_max)

m1_k, m2_k, m3_k, m4_k = st.columns(4)
m1_k.metric("Pillar Strength", f"{p_str:.1f} MPa", delta=f"{t_now:.0f} °C da", delta_color="inverse")
m2_k.metric("Tavsiya: Selek Eni", f"{w_rec:.1f} m")
m3_k.metric("Maks. Cho'kish", f"{s_max_3d*100:.1f} cm")
m4_k.metric("Jarayon bosqichi", f"{'Faol' if time_h < 100 else 'Sovish'}")
...
def generate_integrated_3d(h, layers, s_max):
    ...
    for i, layer in enumerate(layers):
        ...
        fig.add_trace(go.Surface(x=gx, y=gy, z=z_top, colorscale=[[0, layer['color']], [1, layer['color']]], opacity=1.0, showscale=False, name=layer['name'], hoverinfo='skip'))
        fx, fy = gx[-1, :], gy[-1, :]
        fz_t, fz_b = z_top[-1, :], z_bottom[-1, :]
        fig.add_trace(go.Surface(x=np.array([fx, fx]), y=np.array([fy, fy]), z=np.array([fz_t, fz_b]), colorscale=[[0, layer['color']], [1, layer['color']]], opacity=1.0, showscale=False, hoverinfo='text', text=f"Qatlam: {layer['name']}"))
        for side in [0, 1, 2]:
            if side == 0:
                ...
                sx, sy, sz_t, sz_b = gx[0, :], gy[0, :], z_top[0, :], z_bottom[0, :]
            fig.add_trace(go.Surface(x=np.array([sx, sx]), y=np.array([sy, sy]), z=np.array([sz_t, sz_b]), colorscale=[[0, layer['color']], [1, layer['color']]], opacity=1.0, showscale=False, hoverinfo='skip'))
        curr_z = z_bottom_base
    coal_z = -(sum(l['t'] for l in layers[:-1]) + layers[-1]['t']/2)
    u_c, v_c = np.mgrid[0:2*np.pi:20j, 0:np.pi/2:20j]
    r = min(h / 10, 13)
    if r > 1:
        cx_c = r*np.cos(u_c)*np.sin(v_c)
        cy_c = (r*0.8)*np.sin(u_c)*np.sin(v_c)
        cz_c = (r*0.6)*np.cos(v_c) + coal_z
        fig.add_trace(go.Surface(x=cx_c, y=cy_c, z=cz_c, colorscale='Hot', opacity=1.0, showscale=False, name="Yonish kamerasi", lighting=dict(ambient=0.7, diffuse=1.0, specular=0.5)))
    fig.update_layout(
        scene=dict(
...
with st.expander("📝 Avtomatik Ilmiy Interpretatsiya", expanded=True):
    ...
    **Tahlil natijasi ({time_h}-soat):**
    ...
    3. **Xavf darajasi:** **{risk_level}**. Tavsiya etilgan selek eni: **{w_rec:.1f} metr** (AI optimizatsiya: **{optimal_width_ai:.1f} m**).
    """)
...
with t2:
    ...
    st.write(f"**Interpretatsiya:** {T_source_max}°C haroratda jins mustahkamligi {((1 - ucs_t_dyn/ucs_0)*100):.1f}% ga pasaydi.")
