...
def generate_realistic_3d(h, layers, s_max, total_depth, source_z, H_seam):
    ...
    """
    # 1. Grid yaratish (faqat Y ning manfiy qismi uchun - qirqim hosil qilish)
    x = np.linspace(-150, 150, 50)
    y = np.linspace(-100, 0, 25)  # Y=0 gacha bo'lgan qism (qirqim)
    X, Y = np.meshgrid(x, y)
    ...
    # 3. Hajmli Yonish Kamerasi (Zonalar bilan)
    coal_z = curr_z_top + layers[-1]['t']/2
    
    # Zonalar parametrlari (X bo'yicha markazlashgan)
    zones = [
    ...
    for zone in zones:
        u, v = np.mgrid[0:np.pi:15j, 0:np.pi:15j] # Faqat yarim sfera/silindr
        x_z = zone['rad'] * np.cos(u) * np.sin(v) + zone['x']
        y_z = zone['rad'] * np.sin(u) * np.sin(v) - zone['rad'] # Y o'qida siljitish
        z_z = zone['rad'] * 0.6 * np.cos(v) + coal_z
        
        fig.add_trace(go.Mesh3d(
            x=x_z.flatten(), y=y_z.flatten(), z=z_z.flatten(),
            alphahull=0, color=zone['color'], opacity=0.8,
            name=zone['name']
        ))
    ...
    fig.add_trace(go.Scatter3d(
        x=[80, 80], y=[0, 0], z=[10, coal_z-5],
        mode='lines+markers', line=dict(color='silver', width=10),
        marker=dict(size=4, symbol='circle'), name="Production Well"
    ))

    # Permeable Link (Gorizontal kanal)
    fig.add_trace(go.Scatter3d(
        x=[-80, 80], y=[0, 0], z=[coal_z-4, coal_z-4],
        mode='lines', line=dict(color='black', width=6, dash='solid'),
        name="Permeable Link"
    ))
    ...
    fig.update_layout(
        scene=dict(
            ...
            camera=dict(eye=dict(x=1.2, y=-1.8, z=0.8)), # Qirqimga qaragan burchak
            aspectmode='manual',
