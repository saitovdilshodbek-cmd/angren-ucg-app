...
# ====================== YANGI QO'SHIMCHA BLOKLAR (ADVANCED ANALYSIS) ======================
...
# 2. Advanced Analysis Block (to'liq)
def detect_failure_mode(fos, sigma_th, damage, ucs_0):
    if np.mean(fos) < 1.0:
        return "🔴 Shear Failure (global collapse)"
    elif np.max(sigma_th) > 0.6 * ucs_0:
        return "🔥 Thermal Cracking"
    elif np.mean(damage) > 0.5:
        return "🟠 Progressive Damage Failure"
    else:
        return "🟢 Stable"

def decision_system(fos, risk):
    if np.mean(fos) < 1:
        return "🚨 Pillar width oshirish kerak"
    elif np.max(risk) > 0.7:
        return "⚠️ Monitoringni kuchaytirish kerak"
    return "✅ Dizayn optimal"

def compute_risk_ml(fos, damage, void, temp):
    # GradientBoostingRegressor asosida risk (dummy)
    X = np.column_stack([fos.flatten(), damage.flatten(), void.flatten(), temp.flatten()])
    # Dummy model - real training ma'lumotlari bo'lmaganda
    risk_dummy = 0.4*(1-fos/2) + 0.3*damage + 0.2*void + 0.1*(temp/temp.max())
    return np.clip(risk_dummy.reshape(fos.shape), 0, 1)

def hoek_brown_to_mohr_coulomb(ucs, mb, s, sigma3_max=5):
    sigma3 = np.linspace(0, sigma3_max, 50)
    sigma1 = sigma3 + ucs * (mb * sigma3 / ucs + s)**0.5
    coeffs = np.polyfit(sigma3, sigma1, 1)
    k, b = coeffs
    phi = np.degrees(np.arcsin((k - 1)/(k + 1)))
    c = b * (1 - np.sin(np.radians(phi))) / (2 * np.cos(np.radians(phi)))
    return c, phi

# Yangi expanderlar
with st.expander("📊 Advanced Analysis Summary"):
    col1, col2, col3 = st.columns(3)
    col1.metric("O'rtacha FOS", f"{np.mean(fos_2d):.2f}")
    col2.metric("Max Risk", f"{np.max(risk_map):.2f}")
    sigma_th_max = np.max(sigma_thermal)
    col3.metric("Max Thermal Stress", f"{sigma_th_max:.1f} MPa")

    failure_mode = detect_failure_mode(fos_2d, sigma_thermal, damage, ucs_seam)
    st.warning(f"Failure Mode: {failure_mode}")
    decision = decision_system(fos_2d, risk_map)
    st.success(decision)

    # Risk heatmap
    fig_risk = go.Figure(go.Heatmap(z=risk_map, colorscale='Turbo', colorbar=dict(title="Risk 0-1")))
    fig_risk.update_layout(title="Composite Risk Index (FOS+Damage+Void+Temp)", template='plotly_dark', height=400, yaxis=dict(autorange='reversed'))
    st.plotly_chart(fig_risk, use_container_width=True)

with st.expander("📈 FOS vs Temperature & Failure Envelopes"):
    # FOS vs Temperature
    temps = np.linspace(20, T_source_max, 50)
    fos_temp = []
    BETA_CONST = 0.0025
    sigma_v_tot = np.mean(grid_sigma_v)
    for T in temps:
        ucs_t = ucs_seam * np.exp(-BETA_CONST*(T-20))
        p = ucs_t * (rec_width/H_seam)**0.5
        fos_temp.append(p/(sigma_v_tot+EPS))
    fig_ft = go.Figure()
    fig_ft.add_trace(go.Scatter(x=temps, y=fos_temp, mode='lines', name='FOS vs T'))
    fig_ft.add_hline(y=1.0, line_color='red', line_dash='dash')
    fig_ft.add_hline(y=1.5, line_color='green', line_dash='dash')
    fig_ft.update_layout(title="FOS vs Temperature", xaxis_title="Temperature (°C)", yaxis_title="FOS", template='plotly_dark', height=350)
    ...
    s_dyn = grid_s_hb.max()
    sigma3_ax = np.linspace(0, ucs_seam*0.5, 100)
    sigma1_hb = sigma3_ax + ucs_seam * (mb_dyn * sigma3_ax / ucs_seam + s_dyn)**0.5
    c_mc, phi_mc = hoek_brown_to_mohr_coulomb(ucs_seam, mb_dyn, s_dyn)
    sigma1_mc = c_mc * np.cos(np.radians(phi_mc)) + sigma3_ax * np.sin(np.radians(phi_mc))
    fig_env = go.Figure()
    fig_env.add_trace(go.Scatter(x=sigma3_ax, y=sigma1_hb, name="Hoek-Brown", line=dict(color='cyan')))
    fig_env.add_trace(go.Scatter(x=sigma3_ax, y=sigma1_mc, name="Mohr-Coulomb", line=dict(dash='dash')))
    fig_env.update_layout(title="Failure Envelope Comparison", xaxis_title="σ3 (MPa)", yaxis_title="σ1 (MPa)", template='plotly_dark', height=350)
    st.plotly_chart(fig_env, use_container_width=True)

    st.metric("Mohr-Coulomb parameters", f"c = {c_mc:.2f} MPa, φ = {phi_mc:.1f}°")

with st.expander("🔥 Thermal Stress Field & Multi-Physics Index"):
    sigma_thermal_field = E_MODULUS * ALPHA_T_COEFF * (temp_2d - 20)
    fig_th = go.Figure(go.Heatmap(z=sigma_thermal_field, x=x_axis, y=z_axis, colorscale='RdBu', colorbar=dict(title="σ_th (MPa)")))
    fig_th.update_layout(title="Thermal Stress Field", template='plotly_dark', yaxis=dict(autorange='reversed'), height=450)
    st.plotly_chart(fig_th, use_container_width=True)

    # Multi-Physics Risk Index
    mpi = (sigma_thermal_field/ucs_seam)*0.4 + damage*0.3 + (1-fos_2d/2)*0.3
    mpi = np.clip(mpi, 0, 1)
    st.metric("Multi-Physics Risk Index (MPI)", f"{np.mean(mpi):.3f}")
    fig_mpi = go.Figure(go.Heatmap(z=mpi, colorscale='Viridis', colorbar=dict(title="MPI")))
    fig_mpi.update_layout(title="MPI Distribution", template='plotly_dark', height=400, yaxis=dict(autorange='reversed'))
    st.plotly_chart(fig_mpi, use_container_width=True)

with st.expander("🎯 Model Validation & Monte Carlo"):
    # Validation Score
    def validation_score(fos, risk):
        score = 1 - abs(np.mean(fos) - (1 - np.mean(risk)))
        return np.clip(score, 0, 1)
    val_score = validation_score(fos_2d, risk_map)
    st.metric("Model Validation Score", f"{val_score:.3f}")

    # Monte Carlo (soddalashtirilgan)
    ucs_std = st.number_input("UCS standard deviation (MPa)", value=5.0, min_value=0.1)
    gsi_std = st.number_input("GSI standard deviation", value=5.0, min_value=0.1)
    n_mc = st.selectbox("Simulations", [500,1000,2000], index=1)
    @st.cache_data(show_spinner=False)
    def monte_carlo_fos(ucs_mean, ucs_std, gsi_mean, gsi_std, d_mean, temp_mean, H_seam, n_sim=2000):
        ...
        return fos_s, pf
    fos_mc, pf = monte_carlo_fos(ucs_seam, ucs_std, layers_data[-1]['gsi'], gsi_std, D_factor, avg_t_p, H_seam, n_sim=n_mc)
    fig_mc = go.Figure()
    fig_mc.add_histogram(x=fos_mc, nbinsx=40, marker_color=np.where(fos_mc<1.0,'#E74C3C','#27AE60'), name='FOS distribution')
    fig_mc.add_vline(x=1.0, line_color='red', line_dash='dash')
    fig_mc.add_vline(x=1.5, line_color='yellow', line_dash='dash')
    fig_mc.update_layout(template='plotly_dark', height=350, title=f"FOS Distribution | Failure prob: {pf*100:.1f}%")
...
# ====================== ADVANCED ANALYSIS TAB (ilmiy asos) ======================
with tab_advanced:
